...
# Privacy policy — LLM Gateway Browser Control

*Last updated: 2026-09-18*

## The short version

This extension sends your data to **one place: the LLM Gateway you paired it
with yourself.** Whoever runs that gateway holds your data.
...
LLM Gateway is software you or your organisation host. If you did not set up
the gateway yourself, the person or company that did is the one who decides
what happens to what the assistant reads — ask them for their privacy policy,
not ours.

## What the extension sends, and when

Nothing at all until two things are true: you have **paired** a gateway in the
...
While it is on, and only for a conversation you are having in that gateway,
the extension can send the gateway:
...
- **Which gateways you paired**, in Chrome's extension storage, until you
  remove them.
- **Whether the extension is switched on**, and for which gateway, until you
...
The gateway keeps an audit trail of what the assistant did in your browser: who
...
screenshots — is stored by the gateway like the rest of your conversation, and
is passed to whichever AI model that gateway is configured to use. Which model,
and under whose terms, is the gateway operator's decision. That is the part
...
- It does not read your browsing history, your bookmarks, or your saved
  passwords.
- It does not run code sent to it. The gateway sends a fixed set of named
...
feature described above — letting a conversation in *your* gateway act in your
browser — and is transmitted only to the gateway you paired.
...
| `scripting` | To place the small bridge script on the gateway's own page, and to read the structure of pages the assistant works on. |
...
| `storage` | To remember the paired gateway, the on/off state and the activity log — all locally. |
...
gateway you paired — they hold it, and only they can answer.
