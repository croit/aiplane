...
<div class="flex w-full flex-col gap-4">
	...
	{#if data}<div class="flex flex-col items-start gap-4 sm:flex-row"><AdminSettingsRail sections={data.sections} {selected} /><div class="flex min-w-0 grow flex-col gap-4">{#if selected === 'web-search'}{#if search}{#key `${search.provider}:${search.searxng_url}:${search.brave_key_set}`}<SearchSettingsCard {search} onsave={saveSearch} />{/key}{/if}{:else}{#each visibleSections as section (section.name)}<AdminSettingsSection {section} drafts={drafts[section.name] ?? {}} onchange={(key, value) => change(section.name, key, value)} onsave={() => save(section)} onclear={clearField} saving={saving === section.name} />{/each}{/if}</div></div>
