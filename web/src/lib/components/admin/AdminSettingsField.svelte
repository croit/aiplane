...
<div class={field.span === 'full' ? 'flex flex-col gap-1 sm:col-span-2' : 'flex flex-col gap-1'}>
	...
	{:else if field.kind === 'model'}
		...
		{:else}<select id={field.key} class="select select-bordered select-sm w-full" value={draft} onchange={(event) => onchange(field.key, (event.currentTarget as HTMLSelectElement).value)}><option value="">{t('settings-model-automatic')}</option>{#each field.models as model (model)}<option value={model}>{model}</option>{/each}{#if unavailableModel}<option value={draft}>{t('settings-model-unavailable', { model: draft })}</option>{/if}</select>{/if}
	{:else}
