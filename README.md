...
- **TypeSafe System One-compatible API** — `POST /v1/systemone` preserves the typed `noul`, `choice`, and `score` contract, so the official TypeSafe SDK can use the gateway as its base URL while the gateway supplies authentication, aliases, routing, limits, and usage accounting.
