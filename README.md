...
Pick the source on the `/admin/rag` create form — the credential fields come from the provider itself, so the form matches whichever source you choose — and use **Test connection** to check the account and folder before starting an index. `GET /api/v0/rag/providers` returns the same descriptors for scripting.
...
| `/api/v0/*` | session cookie | The rest of the JSON API backing the UI — identity, tokens, chat (including the `GET /api/v0/chat/sessions/{id}/events` stream), memories, scheduled actions, webhooks, skills, integrations, usage, RAG, transcription and speech. ~140 endpoints; the contract is [`docs/openapi.json`](docs/openapi.json), kept in sync with the router by a CI drift test, and the UI's typed client is generated from it. |
...
When Web Push is enabled (the default), the PWA can also deliver **Web Push** notifications: after a user opts in from the `/tokens` screen, the service worker shows a notification when an assistant turn they started finishes while the app isn't focused. This uses a self-generated VAPID keypair (persisted, private half sealed under the at-rest key) and RFC 8291 payload encryption — no third-party push provider or account is involved beyond the browser's own push service.
