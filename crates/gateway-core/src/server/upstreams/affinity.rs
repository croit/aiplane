...
//!
//!   1. **`x-gateway-affinity`**, if the client sends it. Authoritative,
...
///     `x-gateway-affinity` header or from a caller that owns the conversation
...
/// (`ANTHROPIC_CUSTOM_HEADERS="x-gateway-affinity: <something-unique>"`), so one
/// value per terminal is one value per session — an exact answer instead of an
/// inferred one.
pub const AFFINITY_HEADER: &str = "x-gateway-affinity";
...
mod tests {
    ...
    fn nothing_in_the_system_prompt_moves_the_key() {
        ...
        // share a replica. `x-gateway-affinity` is the way out.
