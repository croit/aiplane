...
/// Claim every pending interjection in `ids` that belongs to this
/// conversation, in one statement, and report which ones this caller got.
///
/// One statement rather than a read-check-update per id: the check and the
/// claim have to be the same operation or two tabs can both pass the check,
/// and three round trips per id turned a list from an untrusted body into a
/// multiplier on database work. `RETURNING` reports the winners, so the caller
/// still knows exactly what it holds and what to release if its submit fails.
pub async fn claim_steers(
    pool: &Pool,
    session_id: &str,
    ids: &[String],
    ...
    let sql = format!(
        r#"UPDATE chat_turn_steers
           SET status = 'resent', settled_at = ?
           WHERE status = 'pending'
             AND id IN ({placeholders})
             AND turn_id IN (SELECT id FROM chat_turns WHERE session_id = ?)
           RETURNING id"#
    );
    let mut query = sqlx::query(&sql).bind(Timestamp::now().to_string());
    for id in ids {
        query = query.bind(id);
    }
    let rows = query.bind(session_id).fetch_all(pool).await?;
    rows.iter()
        .map(|row| row.try_get::<String, _>("id").map_err(DbError::from))
        .collect()
}
