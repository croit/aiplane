...
pools-error-name-exists = ein Pool namens `{ $name }` existiert bereits — klicken Sie erneut auf „Pool hinzufügen“, um ihn zu überschreiben, oder ändern Sie den Namen
pools-overwrite-hint = Dieser Name existiert bereits. Erneutes Speichern ÜBERSCHREIBT den vorhandenen Pool — seine Backends, Modelle, Stimmen und Compliance-Flags. Ändern Sie den Namen, um stattdessen einen eigenen Pool anzulegen.
