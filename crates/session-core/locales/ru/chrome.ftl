...
multi-select-family-comfyui = Все рабочие процессы ComfyUI, включая добавленные позже
multi-select-family-mcp = Все инструменты { $subject }, включая добавленные позже
multi-select-mcp-none-cached = Инструменты этого коннектора пока неизвестны — они записываются при первом подключении. Выдайте доступ ко всему коннектору выше или вернитесь после первого использования.
