...
settings-s-content_guard = 内容检查
settings-s-content_guard-blurb = 在发送给所选模型之前检查内容。
settings-f-content_guard-enabled = 启用内容检查
settings-f-content_guard-enabled-help = 只检查所选模型池未覆盖的合规领域。
settings-f-content_guard-model = 检查模型
settings-f-content_guard-model-help = 用于 GDPR 和 NDA 决策的 System One 模型。
settings-content-guard-model-warning = 此 System One 模型不符合：{ $requirements }。发送给它的内容可能会离开相应的保护边界。
content-guard-confirm-heading = 内容保护
content-guard-confirm-question = 此内容被识别为受保护内容。仍要将其发送给所选模型吗？
content-guard-confirm-approve = 仍要发送
content-guard-confirm-deny = 不发送
settings-f-content_guard-mode = 模式
settings-f-content_guard-mode-help = 监控会将决策写入日志；强制执行会应用所选操作。
settings-f-content_guard-mode-opt-monitor = 监控
settings-f-content_guard-mode-opt-enforce = 强制执行
settings-f-content_guard-gdpr_action = GDPR 操作
settings-f-content_guard-gdpr_action-help = 向不符合 GDPR 的池发送个人数据时采取的操作。
settings-f-content_guard-nda_action = NDA 操作
settings-f-content_guard-nda_action-help = 向未受 NDA 覆盖的池发送机密内容时采取的操作。
settings-f-content_guard-gdpr_action-opt-allow = 允许
settings-f-content_guard-gdpr_action-opt-confirm = 请求确认
settings-f-content_guard-gdpr_action-opt-deny = 拒绝
settings-f-content_guard-nda_action-opt-allow = 允许
settings-f-content_guard-nda_action-opt-confirm = 请求确认
settings-f-content_guard-nda_action-opt-deny = 拒绝
