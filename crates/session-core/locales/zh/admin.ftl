...
admin-comfyui-workflow-meta = 标题：{ $title } · 前缀：{ $prefix }
admin-comfyui-parameters = 参数
admin-comfyui-required = 必填
admin-comfyui-recent-jobs = 最近任务
admin-comfyui-pending = { $count } 个等待中
admin-comfyui-job-meta = prompt：{ $prompt } · 创建：{ $created }
admin-comfyui-job-completed = 完成：{ $completed }
