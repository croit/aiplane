...
async fn get_json(http: &reqwest::Client, url: &str, api_key: Option<&str>) -> Probe {
    let mut req = http.get(url).header(
        "user-agent",
        concat!("aiplane/", env!("CARGO_PKG_VERSION"), " detect"),
    );
...
/// AIPLANE_LIVE_OLLAMA=http://localhost:11500/v1 \
...
mod live_ollama {
    ...
    fn base_url() -> String {
        std::env::var("AIPLANE_LIVE_OLLAMA")
            .expect("set AIPLANE_LIVE_OLLAMA to a running Ollama's /v1 base url")
    }

    /// Identification works whether or not anything is loaded — it asks
    /// `/api/version`, which is always there.
    #[tokio::test]
    #[ignore = "needs a running Ollama; set AIPLANE_LIVE_OLLAMA to its /v1 base url"]
