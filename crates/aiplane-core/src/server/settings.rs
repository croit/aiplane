...
/// * `session_key_env` — vestigial. `$AIPLANE_SESSION_KEY` is read directly and
        ...
        let value = match field(key).map(|f| f.kind) {
            Some(Kind::Secret) => match crypto.open_from_string(&value) {
                ...
                None => {
                    ...
                    tracing::error!(
                        "settings.{key} could not be decrypted (AIPLANE_ENCRYPTION_KEY or \
                         AIPLANE_SESSION_KEY changed?); treating it as unset"
                    );
                    continue;
                }
            },
