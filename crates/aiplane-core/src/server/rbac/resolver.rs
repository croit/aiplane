...
/// `ToolRegistry` lives in `aiplane-runtime` and `SkillRegistry` in
/// `aiplane-features`, both above this crate. It's used through generics, not
...
mod tests {
    ...
    /// (`ToolRegistry` in `aiplane-runtime`, `SkillRegistry` in
    /// `aiplane-features`), which a unit test in this crate can't reach:
