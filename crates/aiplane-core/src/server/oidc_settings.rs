...
//!
//! Sibling of [`aiplane_features::server::search_settings`] and the same
...
pub async fn params(pool: &Pool, crypto: &Crypto) -> Result<Option<OidcParams>, DbError> {
    ...
    let Some(client_secret) = crypto.open_from_string(&stored) else {
        ...
        // confusing morning.
        tracing::error!(
            "the stored OIDC client secret could not be decrypted (AIPLANE_ENCRYPTION_KEY \
             or AIPLANE_SESSION_KEY changed?); sign-in is disabled until it is re-entered \
             via `restore-setup`"
        );
        return Ok(None);
    };
