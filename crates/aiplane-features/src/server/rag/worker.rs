...
use aiplane_core::server::db::rag_documents as docs_db;
...
pub struct IndexerConfig {
    ...
    /// serial). See [`aiplane_core::server::config::RagConfig::clone_concurrency`].
...
impl Indexer {
    ...
    /// the office extractor is implemented in `aiplane-runtime`, which sits
    ...
    pub async fn collection_store(
        ...
        collection_id: i64,
        uuid: &str,
    ) -> Result<Pool, aiplane_core::server::db::DbError> {
        ...
        let pool = aiplane_core::server::db::open_collection_store(&path).await?;
        let mut guard = self
            .inner
            .stores
            .lock()
    ...
    async fn index_items(
        ...
        let store = aiplane_core::server::db::open_collection_store(
