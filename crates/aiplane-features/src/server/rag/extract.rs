...
/// client lives in `aiplane-runtime` and `aiplane-features` must never
...
/// trait's existing shape in `aiplane-runtime`.
...
impl DocumentExtractor {
    ...
    async fn recognize(&self, path: &str, mime: &str, bytes: Vec<u8>) -> Extracted {
        ...
        let meta = UsageMeta {
            ...
            source: aiplane_core::server::db::usage::UsageSource::Indexer,
        };
