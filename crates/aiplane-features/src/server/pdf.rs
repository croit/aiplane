...
/// `pub(crate)`, because `fetch_attachment`'s tests live in `aiplane-tools` —
/// see the note on [`ToolContext::for_test`](aiplane_runtime::server::tools::ToolContext::for_test).
