...
fn input() -> IssueInput {
    IssueInput {
        ...
        system_info: json!({
            "url": "https://aiplane.example.com/admin/settings",
            "console_logs": [
...
async fn github_commits_each_image_to_the_assets_branch_then_files_the_issue() {
    ...
    Mock::given(method("GET"))
        .and(path("/repos/croit/aiplane/git/ref/heads/feedback-assets"))
    ...
    Mock::given(method("POST"))
        .and(path("/repos/croit/aiplane/issues"))
        .respond_with(move |req: &Request| {
            let body: serde_json::Value = serde_json::from_slice(&req.body).expect("json body");
            ...
            ResponseTemplate::new(201).set_body_json(json!({
                "number": 99,
                "html_url": "https://github.com/croit/aiplane/issues/99",
            }))
        })
        .expect(1)
        .mount(&server)
    ...
    assert_eq!(result.url, "https://github.com/croit/aiplane/issues/99");
}
