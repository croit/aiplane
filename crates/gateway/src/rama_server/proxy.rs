...
use gateway_core::server::automatic_routing::{
...
pub async fn list_models(State(state): State<Arc<RamaState>>, req: Request) -> Response {
    ...
    if let Ok(routes) = gateway_core::server::db::automatic_routes::all(&state.db).await {
...
pub async fn retrieve_model(State(state): State<Arc<RamaState>>, req: Request) -> Response {
    ...
    let automatic = if id.is_empty() || !access.allows_model(&id) {
        ...
        gateway_core::server::db::automatic_routes::get(&state.db, &id)
            .await
            .ok()
            .flatten()
            .is_some_and(|route| {
...
fn automatic_route_available(
    state: &RamaState,
    route: &gateway_core::server::db::automatic_routes::AutomaticRoute,
    access: &gateway_core::server::upstreams::PoolAccess,
) -> bool {
    state
        .upstreams
