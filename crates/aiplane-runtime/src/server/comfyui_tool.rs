...
use aiplane_features::server::comfyui::ChatUpdateRegistry;
use aiplane_features::server::comfyui::client::Client;
use aiplane_features::server::comfyui::manifest::WorkflowManifest;
use aiplane_features::server::comfyui::runner::{RunError, Runner};
use aiplane_features::server::comfyui::store::ComfyuiStore;
...
pub struct ComfyuiHandle {
    ...
    pub s3: Option<Arc<aiplane_core::server::config::S3Config>>,
...
impl Tool for ComfyuiWorkflowTool {
    ...
    fn run<'a>(&'a self, ctx: ToolContext, args: Value) -> ToolFuture<'a> {
        Box::pin(async move {
            ...
            let pending = aiplane_features::server::comfyui::jobs::pending_count(&ctx.db)
                .await
                .unwrap_or(0);
            ...
            loop {
                let job = aiplane_features::server::comfyui::jobs::get(&ctx.db, job_id)
                    .await
...
mod tests {
    use super::*;
    use aiplane_features::server::comfyui::manifest::{
...
"#,
    ...
    fn tool_id_is_prefixed_with_comfyui_() {
        ...
        let handle = ComfyuiHandle {
            ...
            chat_updates: aiplane_features::server::comfyui::ChatUpdateRegistry::default(),
        };
