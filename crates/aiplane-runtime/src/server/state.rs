...
use aiplane_features::server::rag::worker::Indexer;
use aiplane_features::server::skills::SkillStore;
...
async fn build_oidc_with_retry(
    params: &aiplane_core::server::auth::oidc::OidcParams,
    public_url: &str,
...
///
/// Every one of these is built from a `aiplane-features` scanner or a client
...
/// need `aiplane-tools`, which this crate deliberately does not depend on.
...
/// naming `TypstRenderTool` means depending on `aiplane-tools`, and that crate
...
pub struct FeatureSurface {
    ...
    pub user_skills: Option<Arc<aiplane_features::server::skills::UserSkillStore>>,
...
pub fn typst_template_metas(config: &Config) -> Vec<crate::server::tools::catalog::TemplateMeta> {
    ...
    match aiplane_features::server::typst::discover_templates(&cfg.templates_dir) {
...
pub struct AppState {
    ...
    /// `$AIPLANE_ENCRYPTION_KEY` / the session secret.
...
impl AppState {
    ...
    pub fn user_skills_enabled(&self) -> bool {
        self.user_skills()
            .as_ref()
            .is_some_and(|s| aiplane_features::server::skills::dir_accessible(s.root()))
    }
    ...
    pub fn skills_dir_inaccessible(&self) -> bool {
        self.skills()
            .as_ref()
            .is_some_and(|s| !aiplane_features::server::skills::dir_accessible(s.dir()))
    }
    ...
    pub fn combined_skills_for(
        ...
        match self.user_skills() {
            Some(user_store) => {
                ...
                Some(Arc::new(
                    aiplane_features::server::skills::combined_registry(&global, &private),
                ))
            }
    ...
    pub async fn allowed_tools_for_user(&self, roles: &[String], user_id: &str) -> Vec<String> {
        ...
        let disabled =
            aiplane_core::server::db::user_tool_prefs::disabled_for_user(&self.db, user_id)
                .await
                .unwrap_or_default();
    ...
    pub fn expand_comfyui_tools(&self, allowed: &mut Vec<String>, role_ids: &[String]) {
        ...
        match self.rbac.grants_comfyui_overlay(role_ids) {
            aiplane_core::server::rbac::resolver::ComfyuiGrant::Wildcard => {
            ...
            aiplane_core::server::rbac::resolver::ComfyuiGrant::Specific(ids) => {
            ...
            aiplane_core::server::rbac::resolver::ComfyuiGrant::None => {}
        }
    }
    ...
    pub async fn reload_runtime(&self) {
        use aiplane_core::server::{oidc_settings, setup};
    ...
    ///
    /// [`SECTIONS`]: aiplane_core::server::settings::SECTIONS
    /// [`apply`]: aiplane_core::server::settings::apply
    pub async fn reload_settings(&self) {
        ...
        if let Some(build) = self.tool_family_builder.clone() {
            ...
            let rebuilt = self
                .tools()
                .with_family_replaced(aiplane_core::server::tool_naming::TYPST_PREFIX, family);
    ...
    fn rebuild_surface(&self, config: &Config) {
        ...
        let (skills, user_skills) = match config.skills.as_ref() {
            Some(cfg) => {
                ...
                let user_store = Arc::new(aiplane_features::server::skills::UserSkillStore::new(
                    users_dir,
                ));
    ...
    pub fn user_skills(&self) -> Option<Arc<aiplane_features::server::skills::UserSkillStore>> {
    ...
    pub fn with_user_skills(
        self,
        user_skills: Arc<aiplane_features::server::skills::UserSkillStore>,
    ) -> Self {
    ...
    pub fn with_push(mut self, push: Arc<aiplane_features::server::push::PushSender>) -> Self {
        self.push = Some(push);
        self
    }
    ...
    pub fn provider_registry(&self) -> &aiplane_features::server::rag::source::ProviderRegistry {
        use aiplane_features::server::rag::source::ProviderRegistry;
    ...
    pub fn pool_access_for(&self, roles: &[String]) -> aiplane_core::server::upstreams::PoolAccess {
        let role_ids = self.rbac.role_ids_for(roles);
    ...
    pub async fn reload_rbac(&self) {
        match aiplane_core::server::db::gateway_groups::load_snapshot(&self.db).await {
            Ok(snap) => self.rbac.reload(snap),
            Err(e) => {
                tracing::warn!(error = %e, "reloading RBAC groups; keeping previous snapshot")
            }
        }
        match aiplane_core::server::db::skill_grants::all(&self.db).await {
