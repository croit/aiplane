...
//! `aiplane-tools` sits above this crate, so this is the lowest layer both
//! callers can share.
...
///
/// The indexer lives in `aiplane-features`, which sits *below* this crate and
...
impl aiplane_features::server::rag::extract::OfficeExtractor for SandboxOfficeExtractor {
    fn extract_office<'a>(
        &'a self,
        ext: &'a str,
        bytes: Vec<u8>,
