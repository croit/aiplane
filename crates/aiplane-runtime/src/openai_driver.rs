...
pub fn build_tool_context(state: &Arc<RamaState>, facts: TurnFacts) -> ToolContext {
    ...
    ToolContext {
        ...
        attachment_reservations: Some(
            aiplane_features::server::chat_attachments::new_reservation_set(),
        ),
        indexer: state.indexer.clone(),
        image_gen: Some(aiplane_features::server::image_gen::ImageGenerator::new(
...
async fn run_one_turn(d: &OpenAiDriver, ctx: SessionContext) -> Result<TurnOutcome, TurnError> {
    ...
    let compaction = aiplane_core::server::db::chat_compactions::get(&d.state.db, &ctx.session_id)
        .await
        .unwrap_or(None);
    ...
    for round in 0..max_rounds {
        ...
        if let Err(err) =
            aiplane_core::server::model_defaults::apply_defaults(&d.state.db, &mut request_body)
                .await
        {
        ...
        'chunks: loop {
            ...
            while let Some(event_bytes) = aiplane_core::server::sse::next_event(&mut byte_buf) {
                let event = String::from_utf8_lossy(&event_bytes);
                for line in event.lines() {
                    let Some(payload) = aiplane_core::server::sse::data_payload(line) else {
                        continue;
                    };
                    ...
                    if let Some(t) = aiplane_core::server::sse::usage_tokens(&v) {
                        round_tokens = t;
                    }
...
async fn build_preferences_section(d: &OpenAiDriver) -> Option<String> {
    use aiplane_core::server::db::user_memories;
...
async fn build_hand_edited_docs_section(d: &OpenAiDriver) -> Option<String> {
    ...
    let docs = aiplane_core::server::db::documents::hand_edited_in_session(&d.state.db, session_id)
        .await
        .unwrap_or_default();
...
fn hand_edited_docs_section(
    docs: &[aiplane_core::server::db::documents::Document],
) -> Option<String> {
    use std::fmt::Write as _;
...
async fn build_mcp_offer_section(
    ...
    let enabled = match d.tool_ctx.session_id.as_deref() {
        Some(session_id) => aiplane_core::server::db::chat_session_tools::enabled_keys_for_session(
            &d.state.db,
            session_id,
        )
        .await
    ...
    for key in &connector_keys {
        ...
        let (name, desc) = match aiplane_core::server::db::mcp_catalog::get(&d.state.db, key).await
        {
...
async fn build_skills_section(d: &OpenAiDriver) -> Option<String> {
    ...
    let loaded: Vec<String> = match d.tool_ctx.session_id.as_deref() {
        Some(session_id) => aiplane_core::server::db::chat_session_skills::loaded_for_session(
            &d.state.db,
            session_id,
        )
        .await
...
fn render_skill_listing(
    registry: &aiplane_features::server::skills::SkillRegistry,
    allowed: &[String],
) -> Option<String> {
    use std::fmt::Write as _;
...
///     ([`aiplane_features::server::ocr::pdf_needs_ocr`]) — a born-digital
...
async fn enrich_current_message_with_ocr(
    ...
    let attachments = match aiplane_features::server::chat_attachments::round_attachments(
        &d.state.db,
        &ctx.session_id,
    )
    .await
    ...
    for (index, attachment) in attachments.iter().enumerate() {
        let is_pdf = aiplane_features::server::chat_attachments::is_pdf(
            &attachment.mime,
            &attachment.filename,
        );
        ...
        let fetched = match aiplane_features::server::chat_attachments::fetch(
            s3,
            &attachment.turn_id,
            &attachment.filename,
        )
...
fn ocr_activity_result(
    outcome: &Result<
        aiplane_features::server::ocr::OcrOutcome,
        aiplane_features::server::ocr::OcrError,
    >,
...
async fn pdf_layer_needs_ocr(
    ocr: &aiplane_features::server::ocr::OcrService,
    bytes: &[u8],
) -> bool {
    let min_chars = ocr.auto_min_text_chars_per_page();
    let owned = bytes.to_vec();
    let pages = tokio::task::spawn_blocking(move || {
        aiplane_features::server::pdf::extract_text_pages(&owned)
    })
    .await;
    match pages {
        Ok(Ok(pages)) => aiplane_features::server::ocr::pdf_needs_ocr(&pages, min_chars),
        Ok(Err(error)) => {
...
fn ocr_context_block(
    filename: &str,
    outcome: &aiplane_features::server::ocr::OcrOutcome,
) -> String {
...
fn build_history_messages(
    ...
    compaction: Option<&aiplane_core::server::db::chat_compactions::Compaction>,
...
mod tests {
    ...
    fn active_skills_reinject_the_full_body() {
        use aiplane_features::server::skills::discover;
    ...
    mod history_fold {
        ...
        fn hand_edited_docs_warn_with_ids_and_no_content() {
            ...
            use aiplane_core::server::db::documents::{Document, DocumentFormat};
