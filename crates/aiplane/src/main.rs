...
//! rama server in `aiplane::rama_server`.
//!
//! Run it with `mise run dev`, which supplies the mandatory
//! `AIPLANE_SESSION_KEY` (see [`load_session_secret`] — the gateway refuses to
...
use aiplane::rama_server::SessionStore;
use aiplane_core::server::{self as srv, Config};
use aiplane_features::server as feat;
use aiplane_runtime::AppState;
...
async fn main() -> anyhow::Result<()> {
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| tracing_subscriber::EnvFilter::new("info,aiplane=info")),
        )
        .init();
    ...
    if !db_path.exists() && !srv::env::is_set("AIPLANE_DB_PATH") {
        tracing::warn!(
            ...
            "no database here yet — creating one. If this deployment already had a database, \
             ...
             gateway and set $AIPLANE_DB_PATH to where it actually lives, or this boots as a \
    ...
    let runtime =
        aiplane_runtime::server::state::RuntimeSettings::new_handle(config.public_url_fallback());
    ...
    if config.geoip.is_some() {
        tool_registry = tool_registry.with(aiplane_tools::lookup_ip::LookupIp);
    ...
    if !image_pools.is_empty() {
        tool_registry = tool_registry.with(aiplane_tools::generate_image::GenerateImage);
    ...
    {
        tool_registry = tool_registry.with(aiplane_tools::edit_image::EditImage);
    ...
    let mut enable_tools = aiplane_tools::enable_tools::EnableTools::from_registry(&tool_registry);
    ...
    if let (Some(store), Some(user_store)) = (skill_store.as_ref(), user_skill_store.as_ref()) {
        tool_registry = tool_registry.with(aiplane_tools::read_skill::ReadSkill::new(
            store.clone(),
            user_store.clone(),
            rbac.clone(),
        ));
    ...
    tracing::info!(
        ...
        secure_cookies =
            aiplane::rama_server::session::secure_cookies(&state.public_url()),
        "session policy",
    );
    ...
    // Office documents are read through the sandbox. `aiplane-features` sits
    ...
    // sent them may be long closed.
    {
        ...
        tokio::spawn(async move {
            aiplane_api::pages::chat::start_pending_turns_at_startup(&state).await;
        });
    }
    ...
    aiplane::rama_server::router::serve(state, SocketAddress::new(addr.ip(), addr.port())).await
}

/// Picks up the session HMAC secret from `$AIPLANE_SESSION_KEY` —
...
fn state_session_key() -> String {
    srv::env::var(srv::config::SESSION_KEY_VAR).unwrap_or_default()
}
...
required environment variable is AIPLANE_SESSION_KEY.";
...
/// [`aiplane_core::server::crypto::Crypto::from_env_or_session`]), defaulted
/// by the container image, or configured through the web UI.
const SESSION_KEY_HELP: &str = "AIPLANE_SESSION_KEY is required: it signs browser sessions and \
     (unless AIPLANE_ENCRYPTION_KEY is set) derives the key that seals every secret \
     stored in the database. Generate one ONCE and keep it for the life of the \
     deployment:\n\n    AIPLANE_SESSION_KEY=$(openssl rand -hex 32)\n\n\
...
fn load_session_secret(raw: &str) -> anyhow::Result<[u8; 32]> {
    ...
    let bytes = srv::crypto::hex_decode(raw).ok_or_else(|| {
        anyhow::anyhow!("AIPLANE_SESSION_KEY must be 64 hex chars (32 bytes). {SESSION_KEY_HELP}")
    })?;
    if bytes.len() != 32 {
        anyhow::bail!(
            "AIPLANE_SESSION_KEY decoded to {} bytes, expected 32",
            bytes.len()
        );
    }
...
/// has been using.
///
/// `AIPLANE_DATA_DIR` moved the *default* database path from the working
...
/// set `$AIPLANE_DB_PATH` explicitly chose it and is not affected), the default has
/// actually moved (`AIPLANE_DATA_DIR` is set), a database exists where the old
...
fn refuse_to_orphan_an_existing_database(resolved: &std::path::Path) -> anyhow::Result<()> {
    ...
    anyhow::bail!(
        "found an existing database at {} (the working directory), but this release resolves \
         the default database to {}.\n\nThat path changed when AIPLANE_DATA_DIR was \
         ...
         location (with its -wal and -shm siblings), or point AIPLANE_DATA_DIR / \
         AIPLANE_DB_PATH at where it already lives.",
        legacy.display(),
        resolved.display(),
    )
}
...
mod tests {
    ...
    fn missing_session_key_refuses_to_boot() {
        ...
        assert!(msg.contains("AIPLANE_SESSION_KEY is required"), "{msg}");
