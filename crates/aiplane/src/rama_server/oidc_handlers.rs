...
//!
//! Re-implements the axum version (`aiplane_core::server::api::auth`) on top of
...
use aiplane_core::rama_server::session::{clear_cookie, read_cookie, secure_cookies};
use aiplane_core::server::auth::pending::{self, BINDING_COOKIE, Purpose, clear_binding_cookie};
...
pub async fn login(
    ...
    let return_to = params
        .return_to
        .filter(|rt| aiplane_core::rama_server::session::is_safe_return_to(rt));
