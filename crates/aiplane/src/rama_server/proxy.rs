...
//!
//! Reuses the `UpstreamRegistry` from `aiplane_core::server::upstreams` verbatim
//! — that module has no axum coupling. Only the request/response edges are
//! rewritten for rama's body model.
//!
//! The header policy mirrors `aiplane_core::server::api::proxy`: client
...
fn image_edit_units(fields: &[MultipartField]) -> Option<f64> {
    aiplane_features::server::image_gen::image_units(
        fields
            .iter()
...
fn proxy_tool_ctx(
    ...
    ToolContext {
        ...
        sandbox_lease: state
            .sandbox_client()
            .clone()
            .map(aiplane_runtime::server::tools::sandbox::SandboxLease::new),
...
async fn forward_one_round_once(
    ...
    let affinity =
        aiplane_core::server::upstreams::affinity::hint_for_request(headers, &body_value);
    let acquired = state
        .upstreams
...
/// path uses (see [`aiplane_core::server::capabilities`]) — so a learned flag is found
/// again. Admin-set `Enabled` values are never overwritten (see
/// [`aiplane_core::server::db::model_defaults::mark_unsupported`]). No-op when the
...
fn maybe_learn_capability(state: &RamaState, model: &str, status: u16, body: &[u8]) {
    ...
    let body_str = String::from_utf8_lossy(body)
        ...
        .collect::<String>();
    let Some(cap) =
        aiplane_core::server::upstreams::error_classify::classify_error(status, &body_str)
    else {
        return;
    };
    ...
    tokio::spawn(async move {
        if let Err(e) =
            aiplane_core::server::db::model_defaults::mark_unsupported(&db, &model_key, cap).await
        {
...
fn loop_error_chunk() -> Bytes {
    Bytes::from(format!(
        "data: {}\n\n",
        json!({"error": {"message": aiplane_runtime::loop_guard::LOOP_MESSAGE, "type": "loop_detected"}})
    ))
}
...
async fn forward_streaming(
    ...
    tokio::spawn(async move {
        ...
        let mut content_guard = aiplane_runtime::loop_guard::LoopGuard::new();
        let mut reasoning_guard = aiplane_runtime::loop_guard::LoopGuard::new();
        ...
        'frames: while let Some(frame) = upstream_stream.next().await {
            ...
            while let Some(event) = aiplane_core::server::sse::next_event(&mut buf) {
...
pub(crate) async fn buffered_with_tools(
    ...
    let comfyui = state
        .comfyui()
        .as_ref()
        .map(|h| aiplane_runtime::server::comfyui_tool::ComfyuiToolSource::new((**h).clone()));
    ...
    let tool_source = aiplane_runtime::server::tools::mcp::manager::CompositeToolSource::new(
        tools.as_ref(),
        user_mcp,
    )
...
async fn drive_streaming_tool_loop_inner(
    ...
    for _round in 0..STREAM_TOOL_LOOP_MAX_ROUNDS {
        ...
        let (acquired, upstream, backend_name, started) = {
            let mut attempt = 0;
            loop {
                let acquired = aiplane_core::server::upstreams::route_or_wait(
                    &state.upstreams,
                    &model,
                    PoolKind::Chat,
                    &access,
...
/// [`route_or_wait`](aiplane_core::server::upstreams::route_or_wait) gives the
...
mod tests {
    ...
    ///
    /// `x-aiplane-affinity` names the caller's session. Upstream it means
    ...
    fn the_affinity_header_is_consumed_not_forwarded() {
        let affinity =
            HeaderName::from_static(aiplane_core::server::upstreams::affinity::AFFINITY_HEADER);
        assert!(!is_request_header_forwarded(&affinity));
        // Including under the spelling it had before the rename, which is
        // still honoured and so must still be stripped.
