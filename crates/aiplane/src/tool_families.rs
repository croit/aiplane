...
//! the concrete tool types needs `aiplane-tools`, which `aiplane-runtime` (where
//! the state lives) deliberately does not depend on — hence a callback handed in
//! from this crate, which can see everything.

use aiplane_runtime::server::state::ToolFamilyBuilder;
...
/// naming these tool types needs `aiplane-tools`, which `aiplane-runtime`
...
pub fn typst() -> ToolFamilyBuilder {
    std::sync::Arc::new(|config, surface| {
        ...
        let mut family: Vec<std::sync::Arc<dyn aiplane_runtime::server::tools::Tool>> = Vec::new();
        for t in templates {
            let t = std::sync::Arc::new(t);
            let pptx = t.pptx.is_some();
            family.push(std::sync::Arc::new(
                aiplane_tools::typst_render::TypstRenderTool::new(t.clone(), sandbox.clone()),
            ));
            family.push(std::sync::Arc::new(
                aiplane_tools::typst_render::TypstEditTool::new(t.clone(), sandbox.clone()),
            ));
            family.push(std::sync::Arc::new(
                aiplane_tools::typst_render::TypstReadTool::new(t.clone()),
            ));
            ...
            if let (true, Some(sb)) = (pptx, sandbox.clone()) {
                family.push(std::sync::Arc::new(
                    aiplane_tools::typst_render::TypstPptxTool::new(t.clone(), sb),
                ));
            }
        }
