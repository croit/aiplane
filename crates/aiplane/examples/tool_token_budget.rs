...
use aiplane_runtime::server::tools::{ToolRegistry, catalog};
...
fn main() {
    ...
    match aiplane_features::server::typst::discover_templates(templates_dir) {
        Ok(templates) => {
            for t in templates {
                ...
                registry = registry
                    .with(aiplane_tools::typst_render::TypstRenderTool::new(
                        t.clone(),
                        None,
                    ))
                    .with(aiplane_tools::typst_render::TypstEditTool::new(t, None));
            }
        }
