...
use aiplane_core::server::db::{rag as rag_db, rag_oauth as oauth_db};
...
/// "Connect".
async fn drive_collection(state: &aiplane::rama_server::RamaState) -> i64 {
...
async fn a_drive_collection_saves_before_it_is_connected() {
    ...
    assert!(
        state
            ...
            .unwrap()
            .build(
                &aiplane_features::server::rag::source::ProviderConfig::new(
                    c.source.config.clone(),
                    [("client_secret".to_string(), "s3cret".to_string())]
                        .into_iter()
                        .collect(),
                ),
                &aiplane_features::server::rag::source::ProviderContext::new(
                    reqwest::Client::new(),
                ),
            )
            .is_err(),
