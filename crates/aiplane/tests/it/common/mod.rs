...
pub async fn state_with_alias(upstream_url: &str, alias: &str, real_model: &str) -> RamaState {
    state_with_alias_for_kind(upstream_url, PoolKind::Chat, alias, real_model).await
}

pub async fn state_with_alias_for_kind(
    upstream_url: &str,
    kind: PoolKind,
    alias: &str,
    real_model: &str,
...
pub const TEST_S3_ACCESS_KEY_ENV: &str = "AIPLANE_TEST_S3_ACCESS_KEY";
pub const TEST_S3_SECRET_KEY_ENV: &str = "AIPLANE_TEST_S3_SECRET_KEY";
...
pub async fn state_with_admin_rbac_and_comfyui(upstream_url: &str) -> RamaState {
    use aiplane_features::server::comfyui::{Client, ComfyuiStore};
