...
//!
//! The unit tests in `aiplane-core` pin the registry and the enforcer in
...
async fn state_with_two_models(
    upstream: &MockServer,
) -> aiplane_runtime::rama_server::state::RamaState {
...
async fn session_for(state: &aiplane_runtime::rama_server::state::RamaState, user: &str) -> String {
...
async fn token_management_details_return_saved_scopes_quotas_policy_and_account() {
    ...
    .await
    .unwrap();
    aiplane_core::server::db::user_mcp::set_token_policy(
        &state.db,
        &token_id,
        "*",
        aiplane_core::server::db::user_mcp::AskOverApi::Allow,
    )
    .await
    .unwrap();
