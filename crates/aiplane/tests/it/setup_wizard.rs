...
use aiplane::rama_server::{RamaState, SessionStore, router::service};
use aiplane_core::server::config::Config;
use aiplane_core::server::db::{self, app_settings, gateway_groups, users};
use aiplane_core::server::oidc_settings;
...
use aiplane_runtime::server::state::RuntimeSettings;
...
/// walking screen 1.
fn test_params() -> aiplane_core::server::auth::oidc::OidcParams {
    aiplane_core::server::auth::oidc::OidcParams {
...
async fn a_probe_that_lands_after_setup_closed_is_discarded() {
    ...
    let start = aiplane_core::server::auth::oidc::AuthorizationStart {
        url: "https://id.example.com/auth".into(),
        csrf: "csrf-value".into(),
        nonce: "nonce-value".into(),
        pkce_verifier: "verifier".into(),
    };
    aiplane_core::server::auth::pending::insert(
        &state.db,
        &start,
        Some("/setup"),
        aiplane_core::server::auth::pending::Purpose::Setup,
    )
    .await
    .unwrap();
    mark_configured(&state).await;
