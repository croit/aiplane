...
async fn setup() -> (std::sync::Arc<aiplane::rama_server::RamaState>, String) {
...
async fn setup_state(
...
) -> (std::sync::Arc<aiplane::rama_server::RamaState>, String) {
    ...
    aiplane_core::server::db::gateway_groups::upsert_group(&state.db, "admin", "", true, false)
        .await
        .unwrap();
    aiplane_core::server::db::gateway_groups::set_mappings_for_group(
        &state.db,
        "admin",
        &["admin".to_string()],
    )
...
"#,
    ...
    let job_id = aiplane_features::server::comfyui::jobs::create(
        &state.db,
        "prompt-1",
        "session-1",
        "turn-1",
...
async fn users_roster_includes_resolved_roles_impersonation_state_and_audit() {
    use aiplane_core::server::db::{audit, users};
...
async fn admin_skills_preserve_detail_download_and_effective_grants() {
    ...
    .unwrap();
    state.rbac.reload(
        aiplane_core::server::db::gateway_groups::load_snapshot(&state.db)
            .await
            .unwrap(),
    );
    aiplane_core::server::db::skill_grants::set_for_skill(&state.db, "*", &["admin".to_string()])
        .await
        .unwrap();
    aiplane_core::server::db::skill_grants::set_for_skill(
        &state.db,
        "release-helper",
        &["user".to_string()],
    )
    ...
    state.rbac.set_skill_grant_overlay(
        aiplane_core::server::db::skill_grants::all(&state.db)
            .await
            .unwrap(),
    );
    let app = common::app((*state).clone());
    ...
    assert_eq!(
        aiplane_core::server::db::skill_grants::roles_for_skill(&state.db, "release-helper")
            .await
            .unwrap(),
        vec!["user".to_string()]
    );
...
async fn workspace_surfaces_round_trip() {
    ...
    aiplane_runtime::server::webhooks::record_run_start(
        &state.db,
        hook_id,
        "session-1",
        "summarise",
...
async fn skills_and_connectors_surfaces() {
    ...
    let modes = aiplane_core::server::db::user_mcp::tool_modes(&state.db, "pleb2", "probe")
        .await
        .unwrap();
    assert_eq!(
        modes.get("probe_search"),
        Some(&aiplane_core::server::db::user_mcp::ToolMode::Ask)
    );
