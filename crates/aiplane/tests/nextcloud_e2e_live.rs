...
fn entries(listing: &DirListing) -> Vec<aiplane_features::server::rag::source::RemoteEntry> {
    match listing {
...
async fn the_walker_recurses_a_real_tree_and_prunes_on_the_second_pass() {
    ...
    use aiplane_features::server::rag::source::tree;
    use aiplane_features::server::rag::walk::Filter;
