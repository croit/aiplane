...
// sandbox-runner (RUNNER_URL) and real S3 (AIPLANE_S3_* env + a bucket).
//
...
use aiplane_core::server::config::{S3Config, SandboxConfig};
use aiplane_core::server::db;
use aiplane_features::server::chat_attachments::{self, UploadOutcome};
use aiplane_runtime::server::tools::sandbox::{EditPresentation, RunInSandbox, SandboxClient};
use aiplane_runtime::server::tools::{Tool, ToolContext};
use serde_json::json;
...
fn s3_cfg() -> Arc<S3Config> {
    Arc::new(S3Config {
        ...
        bucket: std::env::var("E2E_S3_BUCKET").unwrap_or_else(|_| "aiplane".into()),
        access_key: None,
        secret_key: None,
        // A live run takes its credentials from the environment it is started
        // with, which the legacy `*_env` path still supports.
        access_key_env: Some("AIPLANE_S3_ACCESS_KEY".into()),
        secret_key_env: Some("AIPLANE_S3_SECRET_KEY".into()),
...
fn client() -> Arc<SandboxClient> {
    ...
    SandboxClient::new(
        ...
        aiplane_runtime::server::state::RuntimeSettings::new_handle("http://localhost:8080"),
    )
}
...
use aiplane_runtime::server::tools::sandbox::{BrowsePage, SandboxLease};
