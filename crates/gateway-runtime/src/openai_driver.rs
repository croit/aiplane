...
async fn run_one_turn(d: &OpenAiDriver, ctx: SessionContext) -> Result<TurnOutcome, TurnError> {
    ...
    let routing_has_tools = !d
        ...
        .await
        .is_empty()
        || !gateway_core::server::db::chat_session_tools::enabled_keys_for_session(
            &d.state.db,
            &ctx.session_id,
        )
        .await
    ...
    let automatic_decision = d
        ...
        .select(
            ...
            &access,
            Some(
                gateway_core::server::automatic_routing::AutomaticRouteAffinity {
...
fn emit_selector_usage(
    d: &OpenAiDriver,
    user_email: &str,
    decision: &gateway_core::server::automatic_routing::AutomaticRouteDecision,
) {
    ...
    d.state.usage.emit(UsageRecord {
        ...
        enforce_limits: d.state.upstreams.enforce_limits_for_model(
            ...
            gateway_core::server::upstreams::PoolKind::SystemOne,
        ),
    });
}
