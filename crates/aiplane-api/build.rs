...
//! Embeds the git commit the binary was built from into `AIPLANE_GIT_SHA`
...
fn main() {
    let sha = std::env::var("CI_COMMIT_SHA")
        ...
        .or_else(|| {
            ...
            // `aiplane_features::server::rag::git::INHERITED_GIT_VARS`; a
    ...
    println!("cargo:rustc-env=AIPLANE_GIT_SHA={sha}");
