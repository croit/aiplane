...
/// `aiplane::rama_server::api`'s envelope — one `/api/v0` contract.
...
fn login_redirect(req: &Request) -> Response {
    if req.method() == Method::GET
        ...
        && aiplane_core::rama_server::session::is_safe_return_to(path_and_query)
...
// `aiplane_core::server::settings`, not hand-built per block.
