...
use aiplane_core::server::db::documents::DocumentFormat;
...
mod tests {
    ...
    async fn pool_with_session() -> aiplane_core::server::db::Pool {
    ...
    fn ctx(pool: aiplane_core::server::db::Pool, session: Option<&str>) -> ToolContext {
        ToolContext {
            user_id: "u1".into(),
            roles: vec![],
    ...
    // lives, in `aiplane_features::server::file_refs`. What stays here is
