...
//! - **PDF**: four model-driven tiers (see [`aiplane_features::server::pdf`]).
...
//!   (`aiplane_features::server::ocr`) and returns recognised text,
...
async fn extract_office(
    ...
    ctx: &ToolContext,
    s3: &aiplane_core::server::config::S3Config,
    turn_id: &str,
    ext: &str,
    bytes: Vec<u8>,
) -> Result<Value, ToolError> {
    let extracted =
        aiplane_runtime::server::tools::sandbox::run_office_extractor(sandbox, ext, bytes).await?;
...
async fn read_ocr(
    ...
    let meta = aiplane_features::server::ocr::UsageMeta {
        ...
        source: if ctx.session_id.is_some() {
            aiplane_core::server::db::usage::UsageSource::Chat
        } else {
            aiplane_core::server::db::usage::UsageSource::V1Api
        },
    };
    let outcome = ocr
...
mod tests {
    ...
    async fn a_canvas_document_id_returns_its_text_and_version() {
        use aiplane_core::server::db::documents::{self, DocumentFormat, VersionAuthor};
    ...
    use aiplane_features::server::pdf::test_support::{blank_pdf, hello_pdf};
    ...
    async fn pdf_text_range_returns_only_the_requested_pages() {
        let out = read_pdf(
            ...
            "doc.pdf",
            "application/pdf",
            aiplane_features::server::pdf::test_support::multipage_pdf(6),
            FetchMode::Text,
            4096,
    ...
    async fn pdf_text_range_marks_page_numbers_for_citation() {
        let out = read_pdf(
            ...
            "doc.pdf",
            "application/pdf",
            aiplane_features::server::pdf::test_support::multipage_pdf(3),
            FetchMode::Text,
            4096,
    ...
    async fn pdf_text_range_tells_the_model_how_to_continue() {
        let out = read_pdf(
            ...
            "doc.pdf",
            "application/pdf",
            aiplane_features::server::pdf::test_support::multipage_pdf(10),
            FetchMode::Text,
            4096,
    ...
    async fn pdf_text_range_past_the_end_is_an_arg_error() {
        let err = read_pdf(
            ...
            "doc.pdf",
            "application/pdf",
            aiplane_features::server::pdf::test_support::multipage_pdf(2),
            FetchMode::Text,
            4096,
    ...
    async fn pdf_text_range_stops_at_the_byte_cap_and_says_so() {
        ...
        let out = read_pdf(
            ...
            "doc.pdf",
            "application/pdf",
            aiplane_features::server::pdf::test_support::multipage_pdf(20),
            FetchMode::Text,
            60,
