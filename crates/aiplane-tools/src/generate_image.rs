...
//! `kind = "image"` upstream pool (via [`aiplane_features::server::image_gen`]), gets the
...
use aiplane_features::server::image_gen::UsageMeta;
...
impl Tool for GenerateImage {
    ...
    fn run<'a>(&'a self, ctx: ToolContext, args: Value) -> ToolFuture<'a> {
        Box::pin(async move {
            ...
            let base = format!(
                "generated-image{}",
                aiplane_features::server::chat_attachments::ext_for_mime(&image.mime)
                    .unwrap_or(".png")
            );
            let filename =
...
mod tests {
    ...
    fn ext_for_mime_maps_common_types() {
        use aiplane_features::server::chat_attachments::ext_for_mime;
