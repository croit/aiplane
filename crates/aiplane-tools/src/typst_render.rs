...
//!   key/value pairs to [`aiplane_features::server::typst::compile`], then splice
...
mod tests {
    use super::*;
    use aiplane_features::server::typst::Field;
    ...
    async fn base_pool() -> aiplane_core::server::db::Pool {
    ...
    fn unused_s3() -> aiplane_core::server::config::S3Config {
    ...
    fn pptx_tool_id_suffixes_render_id_and_requires_base() {
        let cfg = std::sync::Arc::new(aiplane_core::server::config::SandboxConfig {
            enabled: true,
        ...
        let sandbox = SandboxClient::new(
            cfg,
            aiplane_runtime::server::state::RuntimeSettings::new_handle("http://localhost"),
        );
