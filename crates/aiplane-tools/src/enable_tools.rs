...
use aiplane_runtime::server::tools::{Tool, ToolContext, ToolError, ToolFuture, ToolRegistry};
...
pub struct EnableTools {
    ...
    comfyui_store: Option<Arc<aiplane_features::server::comfyui::ComfyuiStore>>,
}
...
impl EnableTools {
    ...
    pub fn with_comfyui_store(
        mut self,
        store: Arc<aiplane_features::server::comfyui::ComfyuiStore>,
    ) -> Self {
...
impl Tool for EnableTools {
    ...
    fn run<'a>(&'a self, ctx: ToolContext, args: Value) -> ToolFuture<'a> {
        Box::pin(async move {
            ...
            let disabled = aiplane_core::server::db::chat_session_tools::disabled_keys_for_session(
                &ctx.db, session_id,
            )
            .await
            .unwrap_or_default();
...
mod tests {
    ...
    fn rag_tools_get_real_copy_not_the_fallback() {
        ...
        let reg = ToolRegistry::new()
            .with(RagListCollections::new(std::sync::Arc::new(
                aiplane_core::server::rbac::Resolver::empty(),
            )))
    ...
    async fn run_refuses_a_key_the_user_disabled() {
        ...
        assert!(!on.contains("fetch_url"));
        assert!(
            aiplane_core::server::db::chat_session_tools::disabled_keys_for_session(&pool, "s1")
                .await
                .unwrap()
                .contains("fetch_url")
        );
    ...
    async fn allowed_tools_for_session_yields_only_bootstrap_before_any_enable() {
        ...
        // prefix).
        aiplane_core::server::db::chat_session_tools::set(&pool, "s1", "fetch_url", true, "model")
            .await
            .unwrap();
        let allowed = state
            .allowed_tools_for_session(&["user".into()], "u1", "s1")
