...
//! (see [`aiplane_core::server::db::documents`]), not in the chat transcript, so
...
use aiplane_core::server::db::documents::{self, DocumentFormat, EditKind};
...
mod tests {
    ...
    async fn seeded_pool() -> aiplane_core::server::db::Pool {
        use std::path::Path;
