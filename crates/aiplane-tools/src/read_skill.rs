...
use aiplane_features::server::skills::{
...
/// obvious import path.
pub use aiplane_runtime::server::tools::catalog::READ_SKILL_ID;
            ...
            match args.path.as_deref() {
                ...
                None => {
                    ...
                    if let Some(session_id) = ctx.session_id.as_deref()
                        && let Err(err) = aiplane_core::server::db::chat_session_skills::record(
                            &ctx.db,
                            session_id,
                            &skill.name,
                        )
...
mod tests {
    ...
    async fn seed_session(pool: &aiplane_core::server::db::Pool, id: &str) {
        sqlx::query(
    ...
    async fn body_call_records_stickiness_for_the_session() {
        ...
        .await
        .unwrap();
        let loaded = aiplane_core::server::db::chat_session_skills::loaded_for_session(&pool, "s1")
            .await
            .unwrap();
    ...
    async fn reading_a_file_does_not_record_stickiness() {
        ...
        .unwrap();
        assert!(
            aiplane_core::server::db::chat_session_skills::loaded_for_session(&pool, "s1")
                .await
                .unwrap()
                .is_empty()
        );
