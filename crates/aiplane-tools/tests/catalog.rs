...
mod catalog_tests {
    ...
    fn capability_domains_derive_from_registry_and_skip_hidden() {
        use aiplane_tools::rag::RagSearch;
        let reg = ToolRegistry::new()
            .with(SearchWeb)
    ...
    fn remember_and_recall_collapse_to_one_memory_entry() {
        use aiplane_tools::memory::{Recall, Remember};
    ...
    fn document_tools_collapse_to_one_document_entry() {
        use aiplane_tools::document::{CreateDocument, EditDocument, ListDocuments, ReadDocument};
        let reg = ToolRegistry::new()
            .with(CreateDocument)
            .with(EditDocument)
            .with(ReadDocument)
