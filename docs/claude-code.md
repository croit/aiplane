# Claude Code through AIplane

Claude Code speaks the **Anthropic Messages API**. AIplane serves it at
`POST /v1/messages`, translating to and from the OpenAI-compatible dialect
every configured upstream already speaks. Point Claude Code at AIplane
and it runs against whatever model you serve — with AIplane's routing,
...
3. **Point Claude Code at AIplane.**
   ...
   `ANTHROPIC_API_KEY` sends it as `x-api-key`. AIplane accepts both, so
   ...
   summaries) to its `haiku` alias, which otherwise reaches AIplane as an
...
`x-aiplane-affinity` names the session outright. Claude Code reads
...
The header is consumed by AIplane and **not** forwarded upstream — it names
...
- While a request is **parked** waiting for a replica to come back
  ([`upstreams.md`](upstreams.md#waiting-out-an-outage)) AIplane sends
...
- Once a streamed turn has started, AIplane emits an SSE `ping` every 15 s,
...
`tool_reference` content blocks. This AIplane does not translate them, so the
default is correct — leave `ENABLE_TOOL_SEARCH` unset. AIplane's own
...
### Putting AIplane models in the `/model` picker
...
offer an AIplane model id under its own name instead, without pretending it is a Claude
model:

```bash
export ANTHROPIC_CUSTOM_MODEL_OPTION=default
export ANTHROPIC_CUSTOM_MODEL_OPTION_NAME="Qwen 3.8 (self-hosted)"
export ANTHROPIC_CUSTOM_MODEL_OPTION_DESCRIPTION="Routed by AIplane to whichever GPU is warm"
```
...
| Variable | Why |
|---|---|
| `CLAUDE_CODE_ENABLE_GATEWAY_MODEL_DISCOVERY=1` | Adds AIplane's models to the `/model` picker. Claude Code calls `GET /v1/models?limit=1000` at startup and keeps entries whose id contains `claude` or `anthropic` — which is what your aliases are for. |
| `CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC=1` | Stops Claude Code reporting telemetry and update checks. Model inference already goes only to AIplane; this also keeps your `/usage` numbers to actual work. |
...
| `ANTHROPIC_CUSTOM_MODEL_OPTION` (+ `_NAME`, `_DESCRIPTION`) | Add an AIplane model id to the `/model` picker under its own name. |
...
| Claude Code sends | AIplane does |
|---|---|
...
tokenizer: AIplane translates the request exactly as it would for
...
The count covers what Claude Code sent. Server-side tools that the loop would
...
- **Thinking-block signatures are ours, not Anthropic's.** A `thinking` block
  AIplane emits carries `"signature": "gateway"`. Nothing verifies it: the
...
- **The repetition guard applies.** Streamed turns run through AIplane's
...
- Features that depend on a claude.ai identity (Remote Control, voice
  dictation) are unavailable while an AIplane credential is set. That is a
  client-side rule, not an AIplane one.
